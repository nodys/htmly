...
var htmly = module.exports = module.exports = function (fileOrBrowserify, opts) {
  if (typeof (fileOrBrowserify) === 'string') {
...
htmly.live = function (server, watcher) {
  ...
  watcher = watcher || (new chokidar.FSWatcher({persistent: true}))
  var watched = []
  var listener = htmly.attachServer(server)
  watcher.on('change', listener)
  htmly.post(function (ctx, done) {
    // Chokidar mays watch a file many times (>=0.10)
    if (watched.indexOf(ctx.filename) === -1) {
      watched.push(ctx.filename)
      watcher.add(ctx.filename)
    }
    done(null, ctx)
  })
...
htmly.attachServer = function (server) {
  ...
  // Attach server only once
  if (!server._htmlyChangeListener) {
    var lrioServer = lrio(server, 'htmly')

    process.htmly.livereload = true

    change.lrioServer = lrioServer
    server._htmlyChangeListener = change
  }
  return server._htmlyChangeListener
}
